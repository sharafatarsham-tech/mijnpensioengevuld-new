...
const AOW_LEEFTIJD_TABEL: Record<number, { leeftijd: number; maanden: number }> = {
 ...
 */
function getAOWIngangsdatum(geboortedatum: Date): Date {
...
function berekenJaarruimte(
  ...
  factorA: number = 0,
  pensioenopbouwWerkgever: number = 0
): {
  jaarruimte: number;
  ...
  const premiegrondslag = Math.min(
    Math.max(brutoJaarinkomen - JAARRUIMTE_2024.franchise, 0),
    JAARRUIMTE_2024.premiegrondslag_max - JAARRUIMTE_2024.franchise
  );
 ...
 * Bereken pensioengat
 */
function berekenPensioengat(
  huidigInkomen: number,
  verwachtPensioen: number,
  gewenstInkomenPercentage: number = 70
): {
  pensioengat: number;
  gewenstInkomen: number;
  maandelijksGat: number;
  analyse: string;
} {
 ...
 */
function berekenBenodidgeInleg(
  doelbedrag: number,
  jarenTotPensioen: number,
  verwachtRendement: number = 4
): {
...
async function handleCalculateAOWAge(args: z.infer<typeof CalculateAOWAgeSchema>): Promise<string> {
...
async function handleCalculateAOWAmount(args: z.infer<typeof CalculateAOWAmountSchema>): Promise<string> {
  const { leefsituatie, opbouwjaren = 50 } = args;
  
  let basisbedrag: number;
  let leefsituatieNL: string;
...
async function handleCalculateJaarruimte(args: z.infer<typeof CalculateJaarruimteSchema>): Promise<string> {
...
async function handleCalculatePensionGap(args: z.infer<typeof CalculatePensionGapSchema>): Promise<string> {
...
async function handleCalculateRequiredSavings(args: z.infer<typeof CalculateRequiredSavingsSchema>): Promise<string> {
...
async function handleCompareScenarios(args: z.infer<typeof CompareScenariosSchema>): Promise<string> {
...
async function handleGeneratePensionReport(args: z.infer<typeof GeneratePensionReportSchema>): Promise<string> {
  const {
    naam,
    geboortejaar,
    geboortemaand = 6,
    ...
    bruto_jaarinkomen,
    type_werknemer,
    werkgeverspensioen_per_jaar = 0,
    eigen_pensioenopbouw = 0,
    aow_opbouwjaren = 50,
    gewenste_pensioenleeftijd = 67,
    gewenst_inkomen_percentage = 70,
    include_advice = true,
  } = args;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      const errorMessages = zodError.issues.map((e: z.ZodIssue) => `${e.path.join('.')}: ${e.message}`).join('\n');
      return {
        content: [{ 
          type: "text", 
